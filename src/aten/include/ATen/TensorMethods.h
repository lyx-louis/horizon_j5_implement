...
inline Tensor & Tensor::reshape_(IntList size, IntList stride) {
    return type().reshape_(*this, size, stride);
}
...
inline bool Tensor::allclose(const Tensor & other, double rtol, double atol) const {
    return type().allclose(*this, other, rtol, atol);
}
...
inline std::tuple<Tensor,Tensor,Tensor,Tensor> Tensor::_det_with_svd() const {
    return type()._det_with_svd(*this);
}
...
inline Tensor Tensor::stft(int64_t frame_length, int64_t hop, int64_t fft_size, bool return_onesided, const Tensor & window, int64_t pad_end) const {
    return type().stft(*this, frame_length, hop, fft_size, return_onesided, window, pad_end);
}
