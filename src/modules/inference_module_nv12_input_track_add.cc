...
inference_module_nv12_input_track_add.cc
...
//  int input_w = 704; // w值
 ...
 //   cv::Mat mat_src_rb(1.5*height_for ,width_for,  CV_8UC1, y_data_rb);
  ...
  memcpy(imgs_data_for_write_tensor,y_data_lf,1.5*imgsize_for); //lb
  cout<<"left front copy"<<endl;
  imgs_data_for_write_tensor += batch_size;
  memcpy(imgs_data_for_write_tensor,y_data_f,1.5*imgsize_for);
  cout<<"front copy"<<endl;
  imgs_data_for_write_tensor += batch_size;
  memcpy(imgs_data_for_write_tensor,y_data_rf,1.5*imgsize_for);
  cout<<"right front copy"<<endl;
  imgs_data_for_write_tensor += batch_size;
  memcpy(imgs_data_for_write_tensor,y_data_lb,1.5*imgsize_for);
  cout<<"left back cp"<<endl;
  imgs_data_for_write_tensor += batch_size;
  memcpy(imgs_data_for_write_tensor,y_data_b,1.5*imgsize_for);
  cout<<" back cp "<<endl;
  imgs_data_for_write_tensor += batch_size;
  memcpy(imgs_data_for_write_tensor,y_data_rb,1.5*imgsize_for);
