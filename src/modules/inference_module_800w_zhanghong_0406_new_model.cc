...
/*
  auto &pym_res0_1 = res[0]->pym_image_->bl_ds_[1];
  int res0width_1 = pym_res0_1.width;
  int res0height_1 = pym_res0_1.height;
  uint8_t *res0y_data_1 = reinterpret_cast<uint8_t *>(pym_res0_1.y_vaddr);
  uint8_t *res0c_data_1 = reinterpret_cast<uint8_t *>(pym_res0_1.c_vaddr);
  std::cout<<"new start"<<std::endl;
  uint8_t * yuv_all_1 = new uint8_t[res0height_1*res0width_1*3/2];
  std::cout<<"new success"<<std::endl;
  memcpy(yuv_all_1,res0y_data_1,res0width_1*res0height_1);
  memcpy(yuv_all_1+res0height_1*res0width_1,res0c_data_1,res0width_1*res0height_1*0.5);
  cv::Mat mat_src_1;
  int mat_src_size_1 = res0height_1*res0width_1*3/2;
  mat_src_1.create(res0height_1*3/2,res0width_1,CV_8UC1);
  memcpy(mat_src_1.data,yuv_all_1,mat_src_size_1);
  cv::Mat mat_dest_1;
  cv::cvtColor(mat_src_1,mat_dest_1,cv::COLOR_YUV2BGR_NV12);
  char fntmp2[1024] = {0};
  sprintf(fntmp2,"chnl0-%02d-Infer_result.jpg",rand());
  cv::imwrite(fntmp2,mat_dest_1);
  delete[] yuv_all_1;
*/
...
void img_trans(cv::Mat& img, cv::Mat& img_crop, cv::Size resize_dims, int crop_w, int crop_h, int fW, int fH) {
    ...
    img_resize(cv::Range(crop_h, crop_h + fH), cv::Range(crop_w, crop_w + fW)).copyTo(img_crop);
}
...
void preprocess(std::vector<cv::Mat>& img_lists, std::vector<cv::Mat>& img_norm_lists) {
    int H = 900; int W = 1600;
    int fH = INPUTHEIGHT; int fW = INPUTWIDTH;
    float resize = fmax(fH / (1.0 * H), fW / (1.0 * W));
    cv::Size resize_dims;
    resize_dims.width = (int)(W * resize);
    resize_dims.height = (int)(H * resize);
    ...
    for (int i = 0; i < img_lists.size(); i++) {
        ...
        img_norm_lists.push_back(img_norm);
    }
...
int InferenceModule::DoProcess(std::vector<spVioMessage> pyramid_messages,
  ...
  ModuleProfile infer_profile_pre("Infer-Pre-Module");
  enum pre_mode {
    PRE_MODE_BIG_RESIZE_SMALL,  //选择比推理图片稍大的PYM层 -> padding ->
                                // RESIZE到推理图片大小，默认
    PRE_MODE_SMALL_PADDING_BIG,  //选择比推理图片稍小的PYM层 -> padding
                                 //到推理图片大小
  };

  //注意：如需改策略则只需修改此定义为上述定义中的某值
  #define PRE_MODE_SMALL_PADDING_BIG
...
//960 540 
  ...
  uint8_t *c_data_0 = reinterpret_cast<uint8_t *>(pym_img_0.c_vaddr);
  uint8_t * yuv_all_0 = new uint8_t[height0*width0*3/2];
  memcpy(yuv_all_0,y_data_0,imgsize0);
  memcpy(yuv_all_0+imgsize0,c_data_0,imgsize0*0.5);
  cv::Mat mat_src0;
  mat_src0.create(height0*1.5,width0,CV_8UC1);
  memcpy(mat_src0.data,yuv_all_0,width0*height0*3/2);    
  cv::Mat mat_dest0; //RGB
  cv::cvtColor(mat_src0,mat_dest0,cv::COLOR_YUV2RGB_NV12); //model input is rgb
  rgb_input_list.emplace_back(mat_dest0);
  delete[] yuv_all_0;
  ...
  int imgsize;
//1024 640  
  for(int i=1;i<pyramid_messages.size();i++){


    auto &pym_img = pyramid_messages[i]->pym_image_->bl_ds_[0];
    
    width = pym_img.width;
    height = pym_img.height;

    imgsize = width*height;
  //  std::cout<<width<<" "<<height<<std::endl;
    uint8_t *y_data = reinterpret_cast<uint8_t *>(pym_img.y_vaddr);
    uint8_t *c_data = reinterpret_cast<uint8_t *>(pym_img.c_vaddr);
    uint8_t * yuv_all = new uint8_t[height*width*3/2];

    memcpy(yuv_all,y_data,width*height);
    memcpy(yuv_all+width*height,c_data,width*height*0.5);
    cv::Mat mat_src;
    mat_src.create(height*1.5,width,CV_8UC1);
    memcpy(mat_src.data,yuv_all,width*height*3/2);    
    cv::Mat mat_dest; //RGB
    cv::cvtColor(mat_src,mat_dest,cv::COLOR_YUV2RGB_NV12); //model input is rgb
    rgb_input_list.emplace_back(mat_dest);
    delete[] yuv_all;
    // propose that the width and height is correct, no need to resize it
  }
...
// test imread
    std::string imgpath0 = "./configs/model/front_left_camera.jpg";
    std::string imgpath1 = "./configs/model/front_camera.jpg";
    std::string imgpath2 = "./configs/model/front_right_camera.jpg";
    std::string imgpath3 = "./configs/model/back_left_camera.jpg";
    std::string imgpath4 = "./configs/model/back_camera.jpg";
    std::string imgpath5 = "./configs/model/back_right_camera.jpg";
    std::cout<<"imwrite"<<std::endl;
    std::vector<string> img_file_path;
    ...
    std::vector<cv::Mat> img_lists(6);

    for (int32_t i{ 0 }; i < 6; i++) {

        cv::Mat frame_mat = cv::imread(img_file_path[i]); // imread ������bgr
        if (frame_mat.empty()) {
            return -1;
        }
        cv::cvtColor(frame_mat, frame_mat, cv::COLOR_BGR2RGB); // 

        rgb_input_list.push_back(frame_mat); // �洢ת�����ͼ
        std::cout<<"read "<<i<<"image success"<<std::endl;
    }
    ...
    for (int32_t i{ 0 }; i < 6;i++) {
        ...
        std::cout << "img_norm_data = " << *img_norm_data << std::endl;
        memcpy(imgs_data_for_write_tensor, img_norm_data, 4*imgsize);
...
//  }
  ...
  */
  ...
  auto output_ptr0 = reinterpret_cast<float*>(output_tensors[0].sysMem[0].virAddr);
  auto output_ptr1 = reinterpret_cast<float*>(output_tensors[1].sysMem[0].virAddr);
  auto output_ptr2 = reinterpret_cast<float*>(output_tensors[2].sysMem[0].virAddr);
...
//  auto output_ptr4 = reinterpret_cast<float*>(output_tensors[4].sysMem[0].virAddr);
    ...
    FILE* pd = NULL;
//    string save_path = "output_fastbev_bin/";
//    string sindex = to_string(ii);
//    string save_name = name_string_same_length(sindex, 6);
//    cout << save_name << endl;
    char fntmp[1024] = {0};

    sprintf(fntmp, "fbtest-%02d-Infer_result.bin", rand());
    //cv::imwrite(fntmp,mat_dest);
    std::string path_total =  "fbtest.bin";
  ...
  //Post process part
    ...
    postprocess(output,intrins_list,trans_list,rots_list);
*/
